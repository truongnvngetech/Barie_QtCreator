/*
- phân tích ảnh đã cắt từng ký tự để đưa ra dự đoán là ký tự là gì
*/
...
bool loadKNNDataAndTrainKNN(void) {
    ...
    cv::FileStorage fsClassifications("E:\\Barie\\qtcreator\\Barie\\classifications.xml", cv::FileStorage::READ);        // open the classifications file

    if (!fsClassifications.isOpened()) {                                                        // if the file was not opened successfully
    ...
    cv::FileStorage fsTrainingImages("E:\\Barie\\qtcreator\\Barie\\images.xml", cv::FileStorage::READ);              // open the training images file
